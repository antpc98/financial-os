...
export function lineChart(container, points, formatter) {
  ...
  container.innerHTML=`<svg viewBox="0 0 ${w} ${h}" role="img"><title>Evolución histórica</title><line class="gridline" x1="${p}" y1="${h-p}" x2="${w-p}" y2="${h-p}"/><path class="line" d="${path}"/>${xy.map(x=>`<circle class="dot" cx="${x.x}" cy="${x.y}" r="4"><title>${esc(x.date)}: ${esc(formatter(x.value))}</title></circle><text x="${x.x}" y="${h-8}" text-anchor="middle">${esc(x.date.slice(5).replace("-","/"))}</text>`).join("")}</svg>`;
}
